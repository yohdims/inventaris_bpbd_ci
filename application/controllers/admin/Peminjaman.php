...
class Peminjaman extends CI_Controller {
	...
	public function index()
	{
		...
		$this->data['peminjaman']=$this->M_Peminjaman->getKembali($this->session->userdata('tgl_sekarang'));
		$this->data['isi'] = $this->load->view($this->link.'/index_admin',$this->data,TRUE);
	...
	public function form($id)
	{
		...
		if($id==0){	
			...
			$this->data['detail_pinjam']=$this->session->set_userdata('detail_pinjam',$this->M_Detail_Pinjam->getAll($id));
	...
	public function set_peminjam(){
		...
		redirect('admin/'.$this->link."/pinjam_barang");
	}
	...
	public function kembali($id_peminjaman)
	{
			...
			if ($this->M_Peminjaman->update_status($id_peminjaman,$update)) {// success
					$this->session->set_flashdata('success_msg', 'Berhasil tambah data');
					redirect('admin/'.$this->link);
				}
			
	}
	public function dipinjam($id_peminjaman)
	{
				$update = array(
				'id_peminjaman'=>$id_peminjaman,
